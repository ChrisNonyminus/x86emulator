...
int main(int, char **) {
  ...
  /*(INIT EMULATOR)*/
  // load bios
  if (!x86emul::LLE::LoadBios("BIOS/W27C512 BIOS AWARD 3262446.BIN", 0xF0000,
                              64 * 1024, 0, 0)) {
    exit(1);
  }

  /*DYNAREC TEST*/
  static uint8_t code[] = {uint8_t(IEmitter::GenericOperation::NOP),
                      uint8_t(IEmitter::GenericOperation::ADD_REG2REG),
                      uint8_t(IEmitter::HostRegister::Arg1),
                      uint8_t(IEmitter::HostRegister::Arg2),
                      uint8_t(IEmitter::HostRegister::ReturnValue),
                      uint8_t(IEmitter::GenericOperation::MOV_REG2MEM),
                      uint8_t(IEmitter::HostRegister::ReturnValue),
                      0,
                      0,
                      0,
                      0,
                      uint8_t(IEmitter::GenericOperation::MOV_MEM2REG),
                      0,
                      0,
                      0,
                      0,
                      uint8_t(IEmitter::HostRegister::Arg1)};
  static TinyCCEmitter emitter;
  
  emitter.RecompileFunction(emitter.ScanFunction(code, sizeof(code)));
  ...
  while (!done) {
    ...
    for (int i = 0; i < x86emul::gMemoryMap.map.size(); i++) {
      memEditors[i].DrawWindow(
          x86emul::gMemoryMap.map[i].name, &x86emul::gMemoryMap.map[i].data[0],
          x86emul::gMemoryMap.map[i].size, x86emul::gMemoryMap.map[i].start);
    }
